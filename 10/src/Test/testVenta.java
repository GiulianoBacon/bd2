...
public class testVenta {
	...
	public static void main(String[] args) {
		try {
			...
			MongoCollection<Document> colventa = db.getCollection("venta");
			...
			venta = new Venta(1, new GregorianCalendar(2020, 10, 15), tarjeta, 340.40,
					...
					filtrarClientePorId(colpersona,"idCliente",1));
			...
			venta = new Venta(2, new GregorianCalendar(2020, 10, 12), efectivo, 450.0,
					...
					filtrarSucursalPorId(sucursalcol, "idSucursal", 1),
					filtrarClientePorId(colpersona,"idCliente",2));
			...
			venta = new Venta(3, new GregorianCalendar(2020, 10, 11), tarjeta, 2350.0,
					...
					filtrarClientePorId(colpersona,"idCliente",4));
			json = gson.toJson(venta);
			ventaDocument = Document.parse(json);
			listaVenta.add(ventaDocument);
			...
			venta = new Venta(5, new GregorianCalendar(2020, 2, 27), tarjeta, 1150.0,
					...
					filtrarSucursalPorId(sucursalcol, idSucursal, 1),
					filtrarClientePorId(colpersona,"idCliente",5));
			json = gson.toJson(venta);
			ventaDocument = Document.parse(json);
			listaVenta.add(ventaDocument);
			...
			venta = new Venta(6, new GregorianCalendar(2020, 1, 30), efectivo, 170.0,
					filtrarEmpleadoPorId(colpersona, "idEmpleado", 5), productosVenta,
					filtrarSucursalPorId(sucursalcol, idSucursal, 3),
					filtrarClientePorId(colpersona,"idCliente",6));
			json = gson.toJson(venta);
			ventaDocument = Document.parse(json);
			listaVenta.add(ventaDocument);
			...
			venta = new Venta(7, new GregorianCalendar(2019, 10, 9), tarjeta, 150.0,
					...
					filtrarSucursalPorId(sucursalcol, idSucursal, 2),
					filtrarClientePorId(colpersona,"idCliente",7));
			json = gson.toJson(venta);
			ventaDocument = Document.parse(json);
			listaVenta.add(ventaDocument);
			...
			venta = new Venta(8, new GregorianCalendar(2020, 11, 29), efectivo, 444.0,
					...
					filtrarSucursalPorId(sucursalcol, idSucursal, 3),
					filtrarClientePorId(colpersona,"idCliente",8));
			json = gson.toJson(venta);
			ventaDocument = Document.parse(json);
			listaVenta.add(ventaDocument);
			...
			venta = new Venta(9, new GregorianCalendar(2020, 8, 10), tarjeta, 333.0,
					...
					filtrarSucursalPorId(sucursalcol, idSucursal, 1),
					filtrarClientePorId(colpersona,"idCliente",9));
			json = gson.toJson(venta);
			ventaDocument = Document.parse(json);
			listaVenta.add(ventaDocument);

			colventa.insertMany(listaVenta);
		} catch (Exception e) {
	...
	private static Cliente filtrarClientePorId(MongoCollection<Document> coleccion, String filtro, int num) {
		Bson bsonFilter = Filters.eq(filtro, num);
		Document documento = coleccion.find(bsonFilter).first();
		Gson gson = new Gson();
		return gson.fromJson(documento.toJson(), Cliente.class);
	}
